...
inline ZZ power(const ZZ& a, long e) { // x = a^e (e >= 0)
	if (e < 0)
		throw std::runtime_error("NTL::power: e < 0");
 ...
 */
inline ZZ XGCD(ZZ a, ZZ b, ZZ &ainv, ZZ &binv)
{
	ZZ g;
...
inline long to_int(const ZZ& z) {
	if (!z.fits_slong_p())
		throw std::runtime_error("NTL::to_int: ZZ doesn't fit in int!");
...
inline void RandomPrime(ZZ& r, long l, long NumTrials=10) {
	if (l <= 1) throw std::runtime_error("NTL::RandomPrime: l out of range");
	if (l == 2) {
		if (RandomBnd(2)) r = 3;
		else r = 2;
		return;
...
inline long SkipWhiteSpace(std::istream& s)
{
	long c;
