...
long SqrRoot(long n)
{
	...
	if (n < 0)
		throw std::runtime_error("negative argument to _ntl_sqrts");
...
long ErrBoundTest(long kk, long tt, long nn)
{
	...
	if (9 * t > NTL_FDOUBLE_PRECISION)
		throw std::runtime_error("ErrBoundTest: t too big");
	...
	if ((t == 2 && k >= 88) || (3 <= t && 9 * t <= k && k >= 21)) {
		if ((1.5 * log2_k + t + 4.0 + n) * fudge
				<= 0.5 * Log2(t) + 2 * (sqrt(t * k)))
			return 3;
	}

	if (k <= 9 * t && 4 * t <= k && k >= 21) {
		if (((log2_3 + log2_7 + log2_k + n) * fudge <= log2_20 + 5 * t)
				&& ((log2_3 + (15.0 / 4.0) * log2_k + n) * fudge
						<= log2_7 + k / 2 + 2 * t)
...
{
	if (k <= 1)
		throw std::runtime_error("GenPrime: bad length");

	if (k > (1L << 20))
		throw std::runtime_error("GenPrime: length too large");
...
void PrimeSeq::shift(long newshift)
{
	...
	if (pshift == 0) {
		...
		if (!movesieve_mem) {
			...
			if (!movesieve_mem)
				throw std::runtime_error("out of memory in PrimeSeq");
		}
...
{
	if (k <= 1) throw std::runtime_error("GenGermainPrime: bad length");

	if (k > (1L << 20)) throw std::runtime_error("GenGermainPrime: length too large");
